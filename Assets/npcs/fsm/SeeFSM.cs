...
namespace Seer
{
    ...
    public class Investigate: StateBase
    {
        ...
        public StateBase Think(AgentBrain brain)
        {
            ...
            {
                Vector3 direction = brain.transform.position - target;
                brain.GetComponent<AiController>().Walk(direction);
            ...
            {
                if (Time.time - time_at_target >= 3.0f)
                {
                    return new Patrol();
                }
            }
    ...
    public class Chase: StateBase
    {
        ...
        {
            target = Utils.GetTargetPos(brain);
            player = SenseManager.GetSenseManager().GetComponent<PlayerMovement>();
        }
    ...
    public class Cooldown: StateBase
    {
        ...
        public void OnEnter(AgentBrain brain)
        {
            brain.GetComponent<AiController>().Stop();
            time_entered = Time.time;
        }

        public StateBase Think(AgentBrain brain)
        {
            if (Time.time - time_entered >= 2.0f)
            {
                return new Patrol();
            }
            return null;
